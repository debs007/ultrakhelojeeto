...
<html lang="en">
  ...
  <form method="post" action="{{route('webconnect.addUsersBulk')}}" enctype="multipart/form-data" id="csvform" style="display: none;">
    ...
    <div class="page">
      ...
      {
        $param = $_GET["fetch"];
        $allUsers = User::where('stockist',$param)->get();
      }
      else
      {
        $allUsers = User::where('type','stockist')->get();
      }
    ...
    @else
      ...
      @endif
      @include('tabler-dev.demo.stockistaddmodal')
          ...
          <div class="container-xl">
            <div class="row g-2 align-items-center">
              <div class="col">
                ...
                <div class="text-secondary mt-1">{{count($allUsers)}} Stockists</div>
              </div>
              <!-- Page title actions -->
                    ...
                    <table class="table card-table table-vcenter text-nowrap datatable">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td><span class="text-secondary">{{++$count}}</span></td>
                          <td>
                            @if(Session::has('super'))
                            <a href="stockistAgents?fetch={{$bet->name}}">{{$bet->name}}</a></td>
                            @else
                            <a href="users?fetch={{$bet->name}}">{{$bet->name}}</a></td>
                            @endif
                          <td>
                          ₹ {{$bet->balance}}
                          </td>
                          ...
                          <td class="text-end">
                            <span class="dropdown">
                              ...
                              <div class="dropdown-menu dropdown-menu-top">
                                ...
                                <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#modal-edit" onclick="EditSet('{{$bet->name}}|{{$bet->percent}}')">
                                  Edit
                                </a>
                                ...
                                @endif
                                <a class="dropdown-item" href="clearUser?id={{$bet->id}}">
                                  Clear user data
                                </a>
                              </div>
                            </span>
