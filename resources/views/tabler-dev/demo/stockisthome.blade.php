    ...
    if(Session::has("super"))
      {
       ...
       // dd($allStockists);
        ...
        {
          $ts = User::where('stockist',$as->name)->where('lastLogin',date('Y-m-d'))->get();
      ...
      else if(Session::has("stk"))
      {
        ...
        $totalUsers = User::where('stockist',$stock)->where('lastLogin',date('Y-m-d'))->get();
      }
      else
      {
        $name = "none";
        $totalUsers = User::select('*')->where('lastLogin',date('Y-m-d'))->get();
      }
	  ...
	  $stockistInfo = User::where('name',$name)->first();
    $totalPlay = 0;$totalWin = 0;$totalEnd = 0;$totalAgentCommision=0;$totalProfit = 0;
      @endphp
      @include('tabler-dev.demo.addmoneystockist')
          ...
          <div class="container-xl">
            <div class="row g-2 align-items-center">
              <div class="col">
                ...
                <div class="text-secondary mt-2">Current Balance : {{$stockistInfo->balance}}</div>
              </div>
              <div class="col" style="text-align:right;">
                
                <!-- <div class="text-secondary mt-1"> <a href="logout?type=ngslTY" class="btn btn-primary">Logout</a></div> -->
                <div class="text-secondary mt-1"> <a href="logout?type=StHYLL" class="btn btn-warning">Logout</a></div>
                <div class="text-secondary mt-1"> <a href="#" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#modal-reset" onclick="OpenAddMoneyPanel('{{$stockistInfo->id}}')">Reset Password</a></div>
              </div>
              <!-- Page title actions -->
          ...
          <div class="container-xl">
            ...
            $countSl = 0;
                        ...
                        <tr>
						  ...
						  <td>
                          @if($bet->isOnline == "no")
                            <span class="badge bg-danger me-10" style="height:0.9rem;width: 0.9rem;"></span>
                            @elseif($bet->isOnline == "yes")
                            <span class="badge bg-success me-10" style="height:0.9rem;width: 0.9rem;"></span>
							@else
							<span class="badge bg-warning me-2" style="height:0.9rem;width: 0.9rem;"></span>
                            @endif
                          </td>
